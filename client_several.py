# КЛИЕНТ - ПРОГРАММА ПО ОБЩЕНИЮ НЕСКОЛЬКИХ КЛИЕНТОВ
...
@log
def create_message(sock, username='Dima', to_user='Dima_1', command=''):
...
def user_interactive(sock, username):
    ...
    while True:
        ...
        command = input(f'Введите сообщение для {to_user} (exit - выход из консольного месседжера): ')

        if command == 'exit':
            send_message(sock, create_exit_message(to_user))
            ...
            create_message(sock, username, to_user, command)
